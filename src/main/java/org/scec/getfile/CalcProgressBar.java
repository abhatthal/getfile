 ...
 * Forecast...").
 *
 * Duplicated from OpenSHA (org.opensha.sha.gui.infoTools)
 * @author Nitin Gupta
 * @author Vipin Gupta
...
class CalcProgressBar extends JDialog {
	 ...
	 * @param message
	 */
	CalcProgressBar(String title, String message) {
	 ...
	 * @param info
	 */
	CalcProgressBar(Component owner, String title, String info) {
		this(owner, title, info, true);
	}
	
	CalcProgressBar(Component owner, String title, String info, boolean visible) {
	...
	void setIndeterminate(boolean indep) {
	 ...
	 * @param total
	 */
	void updateProgress(long count, long total) {
		updateProgress(count, total, null);
	}
	
	void updateProgress(long count, long total, String message) {
		if (total != 0) {
